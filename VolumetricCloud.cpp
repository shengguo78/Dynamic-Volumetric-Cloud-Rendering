...
float Phase( D3DXVECTOR3 vIn, D3DXVECTOR3 vOut )
{
    D3DXVec3Normalize( &vIn, &vIn );
    D3DXVec3Normalize( &vOut, &vOut );
    return (float)(( 1.0 + SQUARE( D3DXVec3Dot(&vIn, &vOut) ) )* 3.0/4.0);
}
...
bool CVolumetricCloud::Setup( LPDIRECT3DDEVICE9 pDevice, Environment *Env, CloudProperties *Cloud )
{
    ...
    D3DXVec3Normalize( &m_vSunlightDir, &m_vSunlightDir );

    m_vViewpoint = *g_pvViewpoint;

    V( m_Simulator.Setup( m_iLength, m_iWidth, m_iHigh ) );
    V( GenerateCloudParticles() );
    V( m_CloudShader.Setup( this, (float)1.2*m_fCellSize, 200 ) );
...
void CVolumetricCloud::UpdateCloudPosition(double fTime)
{
    ...
    D3DXVECTOR3 vDisplacement = m_vWindVelocity * (float)(fTime-m_fTime);
...
{
    D3DXVECTOR3 relation_pos = m_vCloudPos+D3DXVECTOR3((float)(m_fLength/2.0),(float)(m_fHigh/2.0),(float)(m_fWidth/2.0)) - m_vViewpoint;
