...
D3DXVECTOR3* CloudParticle::GetPosition()
{
...
D3DXVECTOR3* CloudParticle::GetPositionFromLastBuffer()
{
...
void CParticlePool::SortbyViewDistances(D3DXVECTOR3 &vLookDir)
{
...
void CParticlePool::UpdateParticlePositions( double fTime )
{
    ...
    D3DXVECTOR3 vDisplacement = m_vWindVelocity * fElapsedTime;
    for( UINT i = 0; i < m_uNumParticles; i ++ )
    {
...
bool CParticlePool::Setup( CVolumetricCloud *pVolumetricCloud, UINT uNumParticles )
{
    ...
    m_pvPositions[0] = new D3DXVECTOR3[uNumParticles];
    if( !m_pvPositions[0] ) return false;

    m_pvPositions[1] = new D3DXVECTOR3[uNumParticles];
...
bool CParticlePool::AddParticle(UINT i, UINT j, UINT k)
{
    ...
    m_pvPositions[0][index] = m_pVolumetricCloud->m_vCloudPos + m_pVolumetricCloud->m_fCellSize * D3DXVECTOR3( (float)i, (float)j, (float)k );
    ...
    m_pvPositions[1][index] = m_pVolumetricCloud->m_vCloudPos + m_pVolumetricCloud->m_fCellSize * D3DXVECTOR3( (float)i, (float)j, (float)k );
